...
func TestClientGetsAllPages(t *testing.T) {
	// given
	gitHubGraphQLAPIMock, noRequests := serverWithPagedResponse()
	...
	// then
	assert.Equal(t, int64(2), noRequests.Load())
}
...
func TestClientCanExecuteRequestsConcurrently(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(fmt.Sprintf("Concurrency:%d", test.concurrentRequests), func(t *testing.T) {
			...
			client := NewGitHubClient(context.TODO(), "", WithConcurrency(int64(test.concurrentRequests)), WithGraphQLAPIURL(gitHubGraphQLAPIMock.URL))
...
func serverWithPagedResponse() (*httptest.Server, *atomic.Int64) {
	var noRequests atomic.Int64
	noRequests.Store(0)
	gitHubGraphQLAPIMock := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if noRequests.Load() == 0 {
			w.WriteHeader(http.StatusOK)
